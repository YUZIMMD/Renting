...
<div class="head-top">
  ...
  <p>&nbsp;</p>
    ...
    <?php
				  ...
				  $sql=mysql_query(" select * from house where id='".$_GET['id']."'");
	...
	<div class="main-image">
		<img src="Picture/house_details/<?php echo $info['details1']?>" alt="Placeholder" class="custom" style="width:610px; height:375px;">
	</div>
...
line-height: 36px;" readonly>
            ...
            <tr>
              <td bgcolor="#FFFFFF" class="y"<span class="y">地址：    
                  <input name="address" type="text" id="address"value="<?php echo $info['address'];?>" style="font-weight:bold; width:400px;" readonly>
              </span></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <input name="mtel" type="text" id="mtel" value="<?php echo $tel;?>" style="font-weight:bold; border:1px solid #999; height:20px"></td>
            </tr>
            <tr>
  ...
  <div class="tab-content">
    ...
    height: 100px;
    margin-left: 70px;
   "></textarea><input type="submit" name="wen" id="wen" value="提交" class="btn btn-info" style="margin-left:10px;position:absolute;top:66px;"></td></tr>
   <?php 
       if(isset($_POST['wen'])==true){
		   if(!$_SESSION['user']!=null){
		echo "<script>alert('房东不能租房哦，请登录普通用户来租房吧');window.location.href='login.php';</script>";
		}else{
	  ...
	  do{
        ...
        <tr>
          <td>
           ...
           <span style="color:#999;"><?php echo $wen['user']?></span>
...
if(isset($_POST['submit'])==true){
	...
	//判断用户是否已经预约过改房屋
	 $sql3=mysql_query(" select * from tb_booking where no='".$_POST['no']."' and user='".$_SESSION['user']."'");
      $info3=mysql_fetch_array($sql3);
	  if($info3){
		  echo "<script>alert('您已经预约该房屋，可以在个人中心中查看哦');window.location.href='mine/index.php?a=房产预约登记';</script>";
		  }else{
	...
	$query=mysql_query("insert into tb_booking(no,name,type,price,muser,mtel,user,tel,word,date)values('".$_POST['no']."','".$_POST['name']."','".$_POST['type']."','".$_POST['price']."','".$result['name']."','".$result['tel']."','".$in['name']."','".$in['tel']."','".$_POST['word']."','".date('Y-m-d',time())."')");
			if($query==true){
				echo "<script> alert('预约成功');window.location.href='mine/index.php?a=房产预约登记';</script>";
			}
			else{
