...
if(isset($_POST['Submit'])==true){
  ...
  if($error5==4){
	  ...
	  $details5 = $uniName5;	  
		...
		$query=mysql_query("update house set no='".$_POST['no']."',name='".$_POST['name']."',price='".$_POST['price']."',type='".$_POST['type']."',img='".$img."',address='".$_POST['address']."',area='".$_POST['area']."',struct='".$_POST['struct']."',rentType='".$_POST['rentType']."',introduce='".$_POST['introduce']."',details1 = '".$details1."',details2 = '".$details2."',details3 = '".$details3."',details4 = '".$details4."',details5 = '".$details5."',status=1 ,uploadTime = '".$time."'where id='".$_GET['id']."'");
		if($query==true){
			if($_GET['exid']==1){
				echo "<script> alert('修改房源成功');window.location.href='admin_index.php?a=已出租房源管理';</script>";
				}else if($_GET['exid']==2){
				echo "<script> alert('修改房源成功');window.location.href='admin_index.php?a=待审核房源管理';</script>";
				}else if($_GET['exid']==3){
				echo "<script> alert('修改房源成功');window.location.href='admin_index.php?a=驳回房源管理';</script>";	
					}else{
			echo "<script> alert('修改房源成功');window.location.href='admin_index.php?a=在线房源管理';</script>";}
		}
	    else{
