...
<form id="form1" name="form1" method="post" action="">
 ...
 mysql_connect("localhost","root","root");
  ...
  include("Connections/conn.php");
	$query=mysql_query("select * from tb_recommend where id='".$_GET['update']."'");
	$result=mysql_fetch_array($query);
?>
    <tr>
      <td height="50" align="center" bgcolor="#FF9966">原来房子编号:
        <input name="no" type="text" id="no" value="<?php echo $result['no'];?>" /></td>
    </tr>
    <tr>
...
if(isset($_POST['tihuan'])==true){
	...
	$ino=$result1['id'];
	$query2=mysql_query("update tb_recommend set ino='".$ino."',no='".$_POST['newno']."'where id='".$_GET['update']."'");
	
	if($query2==true){
			echo "<script> alert('替换成功');window.location.href='admin_index.php?a=推荐页管理';</script>";
		}
	    else{
