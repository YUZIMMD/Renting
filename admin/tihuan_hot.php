...
<form id="form1" name="form1" method="post" action="">
 ...
 mysql_connect("localhost","root","root");
  ...
  include("Connections/conn.php");
	$query=mysql_query("select * from tb_hot where id='".$_GET['update']."'");
...
if(isset($_POST['tihuan'])==true){
	...
	$ino=$result1['id'];
	$query2=mysql_query("update tb_hot set ino='".$ino."',no='".$_POST['newno']."'where id='".$_GET['update']."'");
	
	if($query2==true){
			echo "<script> alert('替换成功');window.location.href='admin_index.php?a=热门页管理';</script>";
		}
	    else{
