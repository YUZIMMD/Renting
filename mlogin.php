				...
				<div class="content" id="ejs_container">
					...
					<div class="form left">
						...
						<div class="form-list dib-wrap ">
							...
							<input id="yhm" name="yhm" data-exp="void-me" type="text" placeholder="手机号/邮箱" class="dib"/>
						</div>
						<div class="form-list dib-wrap ">
						...
						<div class="dib-wrap lh-37">
							<span class="font-bd"><a class="dib" href="mzhuce.php"> 还没有账号？<span class="font-blue">立即注册</span></a></span>
						</div>
					</div>
				</div>
			</form>
...
if(isset($_POST['submit']))
{
	$a=$_POST['yhm'];
	$b=$_POST['mm'];
    $sql="select * from tb_admin where muser='".$a."' and mpwd='".$b."'";//找到表
	...
	if($n==0)
	{
		...
		else
		{
            ...
            $_SESSION['mpwd']=$b;
		echo "<script>";
		echo "alert ('登录成功！欢迎来到HOME，我们将竭诚为您服务');";
		...
		if(isset($_POST['zhuce']))
	{
		echo "<script>";
		echo "alert ('欢迎注册');";
		echo "location.href='mzhuce.php'";
		echo "</script>";
		}
?>
<?php
}else{
	echo "<script>alert('房东和用户不能同时在线，请注销登录再来吧！');window.location.href='index.php';</script>";
}
?>